...
class HttpStatusCodeResponseSerializerTests: XCTestCase {
    ...
    func testSuccess() async throws {
        ...
        let responseStatusCode = try await Mock.ResponseRoute(session: NetworkingSession(urlSession: Mock.UrlSession(mockUrlResponse: mockResponse)),
                                                              serializer: NetworkingSerializers.Response.HttpStatusCode()).task().result.get()
    ...
    func testNilResponse() async throws {
        ...
        let result = await Mock.ResponseRoute(session: NetworkingSession(urlSession: Mock.UrlSession()),
                                              serializer: NetworkingSerializers.Response.HttpStatusCode()).task().result
