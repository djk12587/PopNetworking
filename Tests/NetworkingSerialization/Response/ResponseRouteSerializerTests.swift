...
class ResponseRouteSerializerTests: XCTestCase {

    func testResponseSerializerSerializesSuccessResponse() async throws {
        let serializedValue = try await Mock.ResponseRoute(serializer: Mock.Response.Serializer(.success("mock_success_response"))).run
    ...
    func testResponseSerializerSerializesFailureResponse() async throws {
        ...
        let result = await Mock.ResponseRoute(serializer: Mock.Response.Serializer<Void>(.failure(mockError))).result
