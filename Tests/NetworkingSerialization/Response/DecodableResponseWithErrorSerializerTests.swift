...
class DecodableResponseAndErrorSerializerTests: XCTestCase {
    ...
    func testSuccess() async throws {
        ...
        let responseModel = try await Mock.ResponseRoute(session: NetworkingSession(urlSession: Mock.UrlSession(mockResult: .success(encodedModel))),
                                                         serializer: NetworkingSerializers.Response.DecodableAndError<Mock.DecodableModel, Mock.DecodableError>()).task().result.get()
    ...
    func testDecodingFailure() async throws {
        ...
        let responseResult = await Mock.ResponseRoute(session: NetworkingSession(urlSession: Mock.UrlSession(mockResult: .success(encodedMockErrorModel))),
                                                      serializer: NetworkingSerializers.Response.DecodableAndError<Mock.DecodableModel, Mock.DecodableError>()).task().result
    ...
    func testSuccessAndErrorDecodingFailures() async throws {
        ...
        let responseResult = await Mock.ResponseRoute(session: NetworkingSession(urlSession: Mock.UrlSession(mockResult: .success(unexpectedErrorData))),
                                                      serializer: NetworkingSerializers.Response.DecodableAndError<Mock.DecodableModel, Mock.DecodableError>()).task().result
        XCTAssertThrowsError(try responseResult.get()) { error in

            typealias Serializer = NetworkingSerializers.Response.DecodableAndError<Mock.DecodableModel, Mock.DecodableError>
            guard let serializationError = error as? Serializer.SerializationError else {
                XCTFail("error should be of type Serializer.SerializationError")
                return
            }
            XCTAssertNotNil(serializationError.successTypeDecodingError as? DecodingError)
            XCTAssertNotNil(serializationError.failureTypeDecodingError as? DecodingError)
        }
    }
