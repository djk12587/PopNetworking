...
class NetworkingSessionTests: XCTestCase {
    ...
    func testMockResponse() async throws {
        let mockValue = try await ResponseRoute(baseUrl: "www.mockedExample.com",
                                        serializer: NetworkingSerializers.Response.HttpStatusCode(),
    ...
    func testMockResponsesRunModifiers() async throws {
        ...
        let mockResult = await ResponseRoute(baseUrl: "www.mockedExample.com",
                                     serializer: NetworkingSerializers.Response.HttpStatusCode(),
    ...
    func testInvalidUrlRequestThrowsError() async throws {
        let result = await ResponseRoute(baseUrl: "",
                                 serializer: NetworkingSerializers.Response.HttpStatusCode()).result
    ...
    func testAdaptersPriority() async throws {
        ...
        let highestPriorityAdapter = Mock.Interceptor(adapterResult: .failure(error: highestPriorityAdapterError),
                                                      priority: .highest)
        let result = await Mock.ResponseRoute(session: NetworkingSession(urlSession: Mock.UrlSession(),
    ...
    func testRetriersPriority() async throws {
        ...
        _ = await Mock.ResponseRoute(session: NetworkingSession(urlSession: Mock.UrlSession(),
                             ...
                             serializer: Mock.Response.Serializer<Void>(.failure(NSError(domain: "mock error", code: 0))),
